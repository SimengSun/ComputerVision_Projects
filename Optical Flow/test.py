...
rawVideo = './Easy/TheMartian.mp4'
out = faceTracking.faceTracking(rawVideo)

pdb.set_trace()
vidcap = cv2.VideoCapture('./Easy/TheMartian.mp4')
...
for i in range(145):
    ...
    for k in range(num_bbox):
        cv2.line(image_currentRes, (newbbox[k][0][0], newbbox[k][0][1]), (newbbox[k][1][0], newbbox[k][1][1]), (0, 255, 0), 3)
        cv2.line(image_currentRes, (newbbox[k][0][0], newbbox[k][0][1]), (newbbox[k][2][0], newbbox[k][2][1]), (0, 255, 0), 3)
        cv2.line(image_currentRes, (newbbox[k][1][0], newbbox[k][1][1]), (newbbox[k][3][0], newbbox[k][3][1]), (0, 255, 0), 3)
        cv2.line(image_currentRes, (newbbox[k][3][0], newbbox[k][3][1]), (newbbox[k][2][0], newbbox[k][2][1]), (0, 255, 0), 3)
